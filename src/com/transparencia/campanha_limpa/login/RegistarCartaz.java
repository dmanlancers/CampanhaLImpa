...
public class RegistarCartaz extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		user.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {				
				if( user.getText().toString().length() < 5)
					user.setError(Html.fromHtml("<font color='black'>O nome de utilizador inferior a 5 caracteres!</font>"));
		...
		frmlogin.setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		case R.id.register:
			...
			if(user.getText().toString().length() != 0 /*|| user.getText().toString().length() >= 5*/&& Email.getText().toString().length() != 0 && pass.getText().toString().length() != 0){
				new CreateUser().execute();
			}
			else{	
				Toast.makeText(RegistarCartaz.this, "Preencha os campos em falta!", Toast.LENGTH_LONG).show();
			}
			break;
		case R.id.login1:
			Intent i = new Intent(this, Login_fotoCartaz.class);
			startActivity(i);
			break;
	...
	class CreateUser extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(RegistarCartaz.this);
		...
		protected String doInBackground(String... args) {
			...
			try {
				...
				if (success == 1) {
					Log.d("User Created!", json.getString(TAG_SUCCESS));            	

					Intent i = new Intent(RegistarCartaz.this, Login_fotoCartaz.class);
					finish();
					...
					success = json.getInt(TAG_SUCCESS);
					Log.d("Erro ao criar user!", json.getString(TAG_MESSAGE));
					return json.getString(TAG_MESSAGE);
		...
		protected void onPostExecute(String file_url) {
			...
			pDialog.dismiss();
			if (file_url != null){
				Toast.makeText(RegistarCartaz.this, file_url, Toast.LENGTH_LONG).show();
			}
